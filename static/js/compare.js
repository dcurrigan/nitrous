...
function loadData(data, purchase_data) {
    ...
    function get_data(network, column) {
      ...
      if (column != 'all') {
        var hospital_data = data.filter((obj) => obj[column] === network);
        var hospital_list = [];
        ...
        for (var hosp of hospital_list) {
          var hosp_purchases = purchase_data.filter((obj) => obj.name === hosp);
...
function optionChanged() {   
    ...
    if (selected == 'NMHS') {
      var current_hosp_data = data.filter((obj) => obj.network === selected)
    ...
    function get_line_data(hospital_purchases) {
      ...
      for (let i = 0; i < hospital_purchases.length; i++) {
        ...
        // if a new month, calculate the rate for the preceding month and append to lineData
          ...
          current_rate = get_rate(hospital_purchases[i-1].date, grand_total)
          let newItem = {
                         date:current_month,
                         rate:current_rate
                        }
    ...
    STATEgraphData = get_line_data(purchase_data)
